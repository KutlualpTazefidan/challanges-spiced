...
const titlesWithDuration = recipes.map((recipe) => {
  const result = `${recipe.title} (${recipe.duration}min)`;
  return result;
}); // ['Crepes (60min)', ...]
console.table(titlesWithDuration);

const timePerServing = recipes.map((recipe) => {
  console.log(recipe.duration);
  return recipe.duration / recipe.servings;
}); // [20, 10, 432, 12]
console.log("timesPerServing", timePerServing);
// EXTRA:
...
console.log(recipesThatOnlyTake60minutesOrLess[0]);
